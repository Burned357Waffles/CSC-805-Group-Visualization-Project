...
const PALETTE = [
  "#2563eb", "#7c3aed", "#f97316", "#16a34a", "#0ea5e9",
  "#ef4444", "#a855f7", "#22c55e", "#fb7185", "#14b8a6",
];
...
const POP_EST: Record<string, number> = (() => {
  ...
  US_STATES_50.forEach((s) => {
    ...
    // 0.6M..40M, but keep relative ordering reasonable
    est[s] = Math.round(jitter(seed + 99, 0.6, 40) * 1_000_000);
  });
  // make the “Top 5 by population” realistic enough for demo
...
const REGIONS: Record<string, string[]> = {
  ...
  Mountain: ["Arizona", "Utah", "Colorado", "Nevada", "Idaho", "Montana", "Wyoming", "New Mexico"],
};
...
function genStateSeries(stateName: string): SeriesPoint[] {
  ...
  return MOCK_NATIONAL_TIMELINE.map((row, i) => {
    ...
    // synthesize a “primary series” that lags “any dose” by 6–10%
    const primaryLag = jitter(seed + 333 + i, 6, 10);
    const primary = clamp(any - primaryLag, 0, 100);

    // hesitancy declines with wobble
...
function Chip({
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-2 py-1 text-sm">
      ...
      {label}
      <button onClick={onRemove} className="rounded-full px-1.5 py-0.5 text-slate-500 hover:bg-slate-200">
        ×
      </button>
    </span>
  );
...
function pFromR(r: number, n: number) {
  // two-sided p-value via t with n-2 dof
  if (n < 3) return 1;
  const t = Math.abs(r) * Math.sqrt((n - 2) / (1 - r * r + 1e-9));
  // quick approx of survival function for Student-t
  // (sufficient for UI annotation; replace with precise lib later if desired)
  const a = 0.5 * (1 - erf(t / Math.sqrt(2))); // normal approx
  return Math.min(1, Math.max(0, 2 * a));
}
function erf(x: number) { // Abramowitz-Stegun approximation
  const sgn = x < 0 ? -1 : 1;
  x = Math.abs(x);
...
export default function HesitancyUptake() {
  ...
  const [selected, setSelected] = useState<StateName[]>(["California", "Texas", "Florida", "New York", "Pennsylvania"]);
  const [picker, setPicker] = useState("");
  const [coverage, setCoverage] = useState<CoverageMetric>("any");
  const [week, setWeek] = useState(24); // 1..52, slider drives this
  const [scaleByPop, setScaleByPop] = useState(false);
  const [labelSelected, setLabelSelected] = useState(true); // auto-tuned by count
  ...
  const [softToast, setSoftToast] = useState<{ show: boolean; pending: StateName | null }>({ show: false, pending: null });
  const [hardModal, setHardModal] = useState<{ show: boolean }>({ show: false });
  ...
  const seriesByState = useMemo(() => {
    ...
    US_STATES_50.forEach((s) => m.set(s, genStateSeries(s)));
    return m;
  }, []);

  const weekIdx = Math.max(1, Math.min(52, week)) - 1;

  // scatter points for all states (context) and flag “selected”
  const scatterPoints = useMemo(() => {
    return US_STATES_50.map((s) => {
      const arr = seriesByState.get(s)!;
      const p = arr[weekIdx];
      const cov = coverage === "any" ? p.vaccination_any_pct : p.vaccination_primary_pct;
      return {
  ...
  }, [seriesByState, weekIdx, coverage, selected]);
  ...
  // paths data: array per state with (x,y) across weeks up to Week X
  const pathByState = useMemo(() => {
    const m = new Map<StateName, { state: StateName; color: string; pts: { x: number; y: number; w: number }[] }>();
    US_STATES_50.forEach((s) => {
      const arr = seriesByState.get(s)!;
      const pts = arr.slice(0, weekIdx + 1).map(p => ({
        x: p.hesitancy_pct,
        y: coverage === "any" ? p.vaccination_any_pct : p.vaccination_primary_pct,
        w: p.week,
      }));
      m.set(s as StateName, { state: s as StateName, color: stateColor(s), pts });
    });
    return m;
  }, [seriesByState, weekIdx, coverage]);

  /* -------------------------- LABELS (ONLY CHANGE) ------------------------- */
  // Create a vertically centered Y-axis label that won’t clip.
  // - 'position="insideLeft"' keeps it inside the chart
  // - 'style={{ textAnchor: "middle" }}' centers the text along the axis
  // - 'offset' nudges it rightward to avoid clipping the first letter
  ...
  const addState = (name: string) => {
    if (!name) return;
    if (!isState(name)) return; // guard: ensures 'name' is StateName below
    ...
    if (selected.length >= softCap) {
      // Type is narrowed to StateName here, so pending is type-safe.
  ...
  const setCommon = (key: keyof typeof REGIONS | "Top 5") => {
    const incoming =
      key === "Top 5"
        ? (["California", "Texas", "Florida", "New York", "Pennsylvania"] as StateName[])
        : (REGIONS[key].filter((s) => US_STATES_50.includes(s)) as StateName[]);

    const combined = Array.from(new Set([...selected, ...incoming]));
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px,1fr]">
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-24 lg:h-fit">
            ...
            {softToast.show && (
              <div className="mt-3 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span>You’ve selected 10 states. More may reduce readability.</span>
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-slate-700">
                  States ({selected.length}{selected.length >= hardCap ? `/15 max` : ""})
                </label>
              </div>
              ...
              <div ref={chipRef} className="mt-2 flex flex-wrap gap-2">
                {selected.map((s) => (
                  <Chip key={s} label={s} color={stateColor(s)} onRemove={() => setSelected(selected.filter(x => x !== s))} />
                ))}
              </div>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <datalist id="state-list">
                  {US_STATES_50.map((s) => <option key={s} value={s} />)}
                </datalist>
              </div>

              {/* Quick sets */}
              <div className="mt-3">
                <div className="text-xs font-semibold text-slate-600 mb-1">Select common sets:</div>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    className="rounded-md border border-slate-200 px-2 py-1 text-xs hover:bg-slate-50"
                    title={selected.length >= 10 ? "Adding this set may exceed the 15-state limit." : undefined}
                ...
                <button className="mt-2 text-xs text-slate-500 underline" onClick={() => setSelected([])}>Clear set</button>
              </div>
            </div>

            {/* Coverage metric */}
            <div className="mt-6">
              <label className="text-sm font-semibold text-slate-700">Coverage metric</label>
              <select
                value={coverage}
            ...
            {/* Week slider */}
            <div className="mt-6">
              <label className="text-sm font-semibold text-slate-700">Week ending</label>
              <input
                type="range"
                min={1}
                max={52}
              ...
              />
              <div className="mt-1 text-xs text-slate-600">Week {week} of 52</div>
            </div>
          ...
          <section className="space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-slate-900">
                  State Scatter — Hesitancy % vs Vaccination % (Week {week})
                </h3>
                {hasSelection && (
                  <div className="text-xs text-slate-600">
                    Trend line (OLS): r = {trend.r.toFixed(2)}, p = {trend.pval.toFixed(2)}, n = {trend.n}
                  </div>
                )}
              </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart>
                    ...
                    <XAxis type="number" dataKey="x" domain={[0, 100]} tick={{ fontSize: 11 }}>
                      <Label value="Hesitancy (%)" position="insideBottom" offset={-4} />
                    </XAxis>
                    <YAxis
                      type="number"
                      dataKey="y"
                    ...
                    <Tooltip
                      formatter={(val: any, name: any) => {
                        if (name === "x") return [`Hesitancy %: ${Number(val).toFixed(1)}`, ""];
                        if (name === "y") return [`Coverage %: ${Number(val).toFixed(1)}`, ""];
                        return [val, ""];
                      }}
                      labelFormatter={() => `Week ${week}`}
                    />

                    {/* render nothing when no selection */}
                    {hasSelection && (
                      <>
                        ...
                        <Scatter
                          ...
                          shape={(props: any) => {
                            const { cx, cy } = props;
                            const r = scaleByPop ? Math.max(3, Math.log10((props.payload.pop || 1) / 1e5)) : 4;
                            return <circle cx={cx} cy={cy} r={r} fill="#94a3b8" opacity={0.35} />;
                          }}
                        />
                        ...
                        <Scatter
                          ...
                          shape={(props: any) => {
                            ...
                            const r = scaleByPop ? Math.max(5, Math.log10((payload.pop || 1) / 1e5) + 1.5) : 6;
                            return (
                              <>
                                <circle cx={cx} cy={cy} r={r} fill={payload.color} filter="url(#halo)" />
                                {labelSelected && (
                                  <text x={cx + r + 4} y={cy + 4} fontSize={11} fill="#334155">
                                    {payload.state}
                                  </text>
                                )}
                              </>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h3 className="text-lg font-semibold text-slate-900">
                Longitudinal Paths — Hesitancy % → Coverage % Over Time (Ends Week {week})
              </h3>
              ...
              <div className="mt-3 h-[360px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart>
                    ...
                    {hasSelection && (
                      <>
                        {/* dim all paths for context when many are selected */}
                        {US_STATES_50.map((s) => {
                          const { pts, color } = pathByState.get(s as StateName)!;
                          const isSel = selected.includes(s as StateName);
                          if (selected.length > 10 && !isSel) return null; // hide context when too many
                          return (
                            <Line
                              key={`path-base-${s}`}
                              data={pts}
                        ...
                        {selected.map((s) => {
                          const { pts, color } = pathByState.get(s)!;
              ...
              {hasSelection && (
                <div className="mt-3 rounded-md border border-slate-200 bg-white px-3 py-2">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-x-6 gap-y-2">
                    {selected.map((s) => (
                      <div key={`legend-${s}`} className="flex items-center gap-2">
                        <span
            ...
            <div className="text-xs text-slate-500 flex items-center justify-between px-1">
              <div>Data Sources: CDC COVID Data Tracker, NCHS, HHS (mocked locally for now)</div>
        ...
        >
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Deselect a state to add another, or use Compare States for detailed time-series.
            </p>
